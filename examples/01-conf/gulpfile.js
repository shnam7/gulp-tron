...
tron.task('g5', bs => console.log(`${bs.name}:className=${bs.className}`), { dependsOn: b1, ...taskOptions })

tron.task({
    name: '@build',
    dependsOn: tron.series(b1, tron.parallel(b2, b3, b4), b5, tron.taskName('g5')),
    triggers: tron.series(b1, tron.parallel(b2, b3, b4), b5, tron.taskName('g5')),
})
