...
import { babelP, coffeelintP, coffeeP, concatP, eslintP, terserP } from "../src/index.js";
...
describe("plugin-scripts", () => {
  ...
  describe("concatP", () => {
    ...
    it("throws when called with empty string", () => {
      const bs = mockBs();
      expect(() => concatP("")(bs)).toThrow("concatP: output filename is required");
    });
    it("throws when options object has no path", () => {
      const bs = mockBs();
      expect(() => concatP({} as unknown as string)(bs)).toThrow(
        "concatP: path is required in options",
      );
    });
  });
