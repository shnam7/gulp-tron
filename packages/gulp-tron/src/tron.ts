...
export class Tron {
   ...
   *
   * @param nameOrConfig Task name string, or a TaskConfig object that defines the complete task configuration.
  ...
  task(
    ...
    if (conf.name === undefined || conf.name === null || !isValidTaskName(conf.name))
      throw new Error(`Tron:task: invalid task name: ${conf.name}`);

    this._resolveTaskConfig(conf);
  ...
  addWatcher(options: WatcherOptions = {}): this {
    ...
    this.task({ ...options, name: taskName, build: watcherFunction } as TaskConfig);
  ...
  findTask(name?: string): TaskBlock | undefined {
    return name === undefined ? undefined : this.#taskBlocks.get(name);
  }
  ...
  protected _resolveBuildSet(buildSet?: BuildSet): GulpTaskFunction | undefined {
    if (buildSet === undefined || buildSet === null) return undefined;
    ...
    if (isFunction(buildSet)) {
      ...
      const fnName = (buildSet as BuildFunction).name;
      const functionName = fnName === "" ? "buildFunc" : fnName;
  ...
  protected _resolveTaskConfig(conf: TaskConfig): GulpTaskFunction {
    const { name, build, dependsOn, triggers } = conf;
    ...
    const hasBuild = build !== undefined && build !== null;
    const hasNoDeps = deps === undefined || deps === null;
    const hasNoTrigs = trigs === undefined || trigs === null;
    if (hasBuild || (hasNoDeps && hasNoTrigs)) {
      tasks.push(main);
    }
    ...
    return gulp.task(taskName)?.unwrap() as GulpTaskFunction;
  }
}
